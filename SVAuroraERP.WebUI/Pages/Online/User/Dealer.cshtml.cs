...
{
    public class DealerModel : BasePageModel
    {
        ...
        private const int PageControlID = (int)Common.Pages.Dealer; // ID for this specific page
        public DealerModel(IHSRPUserServiceRepository respository,
        ...
        public JsonResult OnGetDealerByID(int ID)
        {
        ...
        public JsonResult OnGetDealerList(int draw, int start, int length)
        {
            ...
            dataResponse = (_repository.GetDealer());
            ...
            var filteredData = string.IsNullOrWhiteSpace(searchValue)
                ...
                : resultdata.Where(d => d.CompanyName.Contains(searchValue, StringComparison.OrdinalIgnoreCase) ||
                   (d.HSRPUserCode ?? string.Empty).Contains(searchValue, StringComparison.OrdinalIgnoreCase) ||
                                      (d.Pincode ?? string.Empty).Contains(searchValue, StringComparison.OrdinalIgnoreCase) ||
                                     (d.DeliveryStateName ?? string.Empty).Contains(searchValue, StringComparison.OrdinalIgnoreCase) ||
                                     (d.DeliveryPincode ?? string.Empty).Contains(searchValue, StringComparison.OrdinalIgnoreCase) ||
                                     (d.DeliveryDistrict ?? string.Empty).Contains(searchValue, StringComparison.OrdinalIgnoreCase) ||
                                     (d.City ?? string.Empty).Contains(searchValue, StringComparison.OrdinalIgnoreCase) ||
                                     (d.DeliveryCity ?? string.Empty).Contains(searchValue, StringComparison.OrdinalIgnoreCase) ||
            ...
            Func<VHSRPUser, object> orderByFunc = orderCol switch
            {
                ...
                2 => d => d.Pincode,
                3 => d => d.DistrictName,
                4 => d => d.DeliveryPincode,
                5 => d => d.DeliveryStateName,
                6 => d => d.DeliveryDistrict,
                7 => d => d.City,
                8 => d => d.DeliveryCity,
        ...
        public JsonResult OnPostDealerList([FromForm] HSRPUserRequest dataTableRequest)
        {
            try
            {
                ...
                dataTableRequest.UserTypeID = (byte)HSRPUserTypeEnum.Dealer;
                DataResponse dataResponse = new DataResponse();
            ...
            {
                _logger.LogError(ex, "Error loading Dealer data");
