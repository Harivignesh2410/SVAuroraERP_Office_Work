...
{
    public class ApproveOnlineOrdersModel : HSRPBasePageModel
    {
        ...
        private const int PageControlID = (int)Common.Pages.ApproveOnlineOrders;
        private readonly IOnlineHSRPOrderServiceRepository _hsrpOrdersServiceRepository;
        public ApproveOnlineOrdersModel(ILogger<CreateInvoiceModel> logger,
                                        IErrorLoggerService errorLoggerService,
                                        IAuditLogger auditLogger,
                                        IAntiforgery antiforgery,
                                         IPermissionServiceRepository permissionService,
                                            IOnlineHSRPOrderServiceRepository hsrpOrdersServiceRepository)
        {
            _logger = logger;
        ...
        public JsonResult OnPostHSRPOnlineOrderListData([FromForm] OnlineOrderDTRequest dataTableRequest)
        {
            try
            {
                ...
                dataResponse = _hsrpOrdersServiceRepository.GetOnlineOrderList(dataTableRequest);
            ...
            catch (Exception ex)
            {
                ...
                return new JsonResult(new
                {
                    ...
                    recordsTotal = 0,
                    recordsFiltered = 0,
                    data = new List<VOnlineHSRPOrder>()
                });
            }
        }

        public JsonResult OnGetViewOnlineOrderById(int hsrpOrderId)
        {
            var order = _hsrpOrdersServiceRepository
                            .GetOnlineOrderByHSRPOrderID(hsrpOrderId);
        ...
        public JsonResult OnPostApproveOnlineOrders([FromBody] Approvedata request)
        {
            DataResponse dataResponse = new DataResponse();
            try
            {
                request.LastUpdatedBy = (int)HSRPLoggedUser.UserID;
                dataResponse = _hsrpOrdersServiceRepository.ApproveOnlineOrders(request);
