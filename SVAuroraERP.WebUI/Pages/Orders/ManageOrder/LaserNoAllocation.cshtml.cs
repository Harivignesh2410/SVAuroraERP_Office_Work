...
{
    public class LaserNoAllocationModel : HSRPBasePageModel
    {
        private readonly ILogger<LaserNoAllocationModel> _logger;
        ...
        private const int PageControlID = (int)Common.Pages.LaserNoAllocation; // ID for this specific page

        public LaserNoAllocationModel(ILogger<LaserNoAllocationModel> logger,
        ...
        public IActionResult OnGet()
        {
            ...
            {
                return RedirectToPage("/Orders/ViewOrders/ViewLaserNoAllocation");
            }
            return Page();
        }
        public void LoadOEMList()
        {
            ...
            {
                dataResponse = _hsrpuserservicerepository.GetOEMByEmbossingStation(HSRPLoggedUser.HSRPUserID);
            }
            else
            {
        ...
        public JsonResult OnPostCheckAvailableOrderLaserNo([FromBody] CheckAvailableOrderLaserNoRequest request)
        {
            DataResponse dataResponse = new DataResponse();
            try
            {
                ...
                if (request.OrderIds.Length > 0)
                {
                    dataResponse = _laserNoAllocationServiceRepository.CheckAvailableOrderLaserNo(request);
                }
                return new JsonResult(new { success = true, result = dataResponse });
            }
            catch (Exception ex)
