...
$("#btnSave,#btnUpdate").on('click', function () {
    ...
    var DesignationData = new Object();

    DesignationData.DesignationID = 0;
    if (this.id == "btnUpdate" && $("#hdnDesignationID").val() > 0) DesignationData.DesignationID = $("#hdnDesignationID").val();

    DesignationData.DesignationName = $('#txtDesignationName ').val();
    DesignationData.Description = $('#txtDescription').val();
    DesignationData.IsActive = $("#chkActive").is(':checked') ? true : false;

    //Designation Name
    if (!DesignationData.DesignationName) return markInvalid("#txtDesignationName", "Please enter Designation Name"); 

    // If validation fails, keep focus on the first invalid input
    if (!isValid) return;

    SaveandUpdateDesignation(DesignationData);

    return false;
});
function SaveandUpdateDesignation(DesignationData) {
    if (ENABLE_VERBOSE_Logging)// console.log(DesignationData);
            ...
            if (ENABLE_VERBOSE_Logging) //console.log(response);
                if (response != null && response != null) {
                        if (response.resultdata.Success && !response.resultdata.Error) {
                            Swal.fire({
                                title: DesignationData.DesignationID == 0 ? "Saved!" : "Updated!",
                                text: DesignationData.DesignationID == 0 ? SaveSuccessMessage : UpdateSuccessMessage,
                                icon: "success"
                            }).then(() => {
...
function getRecordList() {
    ...
    $('#tblrecordlist').DataTable({
        ...
        "columns": [
            ...
            { "data": "DesignationName", "orderable": true, "width": "15%" },
            ...
            },
            {
                ...
                "className": "text-center",
                render: function (data, type, row) {
                    return SetActionButtons(data.DesignationID, _CMPermissions);
                },
                "width": "5%",
                "orderable": false
            },
    ...
    $.ajax({
        ...
        success: function (response) {
            if (response.resultdata.Value) {
                ...
                var DesignationData = response.resultdata.Value;
                $("#hdnDesignationID").val(DesignationData.DesignationID);
                $("#txtDesignationName").val(DesignationData.DesignationName);
                $("#txtDescription").val(DesignationData.Description);
                $("#chkActive").prop('checked', DesignationData.IsActive);


                $("#divRecordLog").show();
                $("#spnLastUpdatedBy").html("Last Updated By: " + DesignationData.LastUpdatedByName);
                $("#spnLastUpdatedDate").html("Date: " + ISTtoLocalTime(DesignationData.LastUpdatedDateIST));
            }
            else if (!response.resultdata.Success && response.resultdata.Error) {
                Swal.fire({ title: "Error", text: response.resultdata.Message, icon: "error", confirmButtonColor: "#556ee6" });
            }
