...
$(function () {
   ...
   // VehiclePlateColorList()
    GetVehiclePlateColorList("ddlPlateColor", VehiclePlateColorListUrl, _TOKEN);
    GetColorList("ddlColor", ColorListUrl, _TOKEN);
    $("#btnSave").show();
    $("#btnUpdate").hide();
    pLoadingSetup(true);
});
...
$("#btnSave,#btnUpdate").on('click', function () {
    ...
    ColorData.MapInventoryandHSRPColorID = 0;
    if (this.id == "btnUpdate" && $("#hdnMapInventoryandHSRPColorID").val() > 0) ColorData.MapInventoryandHSRPColorID = $("#hdnMapInventoryandHSRPColorID").val();

    ColorData.ColorID = $('#ddlColor').val();
    ColorData.HSRPPlateColorID = $('#ddlPlateColor').val();
    ColorData.IsActive = $("#chkActive").is(':checked') ? true : false;


    if (!ColorData.ColorID) return markInvalid("#ddlColor", "Please Select Color Name");
    if (!ColorData.HSRPPlateColorID) return markInvalid("#ddlPlateColor", "Please Select HSRP Plate Color");
            ...
            success: function (response) {
                ...
                if (response != null && response.result != null) {
                    if (response.result.Success && !response.result.Error) {
                        Swal.fire({
                            title: ColorData.MapInventoryandHSRPColorID == 0 ? "Saved!" : "Updated!",
                            text: ColorData.MapInventoryandHSRPColorID == 0 ? SaveSuccessMessage : UpdateSuccessMessage,
                            icon: "success"
                        }).then(() => {
...
function getRecordList() {
    ...
    $('#tblrecordlist').DataTable({
        ...
        "columns": [
            ...
            { "data": "ColorName", "orderable": true, "width": "10%" },
            ...
            },
            {
                ...
                bSortable: false,
                render: function (data, type, row) {
                    return SetActionButtons(row.MapInventoryandHSRPColorID, _CMPermissions);
                },
                "width": "5%",
                "orderable": false
            },
...
function EditData(id, ViewFlag) {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            $('#divAddEditModal').modal('show');
            var MapInventoryandHSRPColorIDdata = response.Value;

            $("#hdnMapInventoryandHSRPColorID").val(MapInventoryandHSRPColorIDdata.MapInventoryandHSRPColorID);
            $("#ddlColor").val(MapInventoryandHSRPColorIDdata.ColorID).change();
            $("#ddlPlateColor").val(MapInventoryandHSRPColorIDdata.HSRPPlateColorID).change();
            $("#chkActive").prop('checked', MapInventoryandHSRPColorIDdata.IsActive);

            $("#divRecordLog").show();
            $("#spnLastUpdatedBy").html("Last Updated By: " + MapInventoryandHSRPColorIDdata.LastUpdatedByName);
            $("#spnLastUpdatedDate").html("Date: " + ISTtoLocalTime(MapInventoryandHSRPColorIDdata.LastUpdatedDate));
...
$('#divAddEditModal').on('shown.bs.modal', function () {
    $('#ddlPlateColor').select2({ dropdownParent: $('#divAddEditModal'), width: '100%' });
