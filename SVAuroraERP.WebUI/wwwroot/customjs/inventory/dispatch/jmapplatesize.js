...
$(function () {
    ...
    getRecordList();
    GetVehiclePlateSizeList("ddlHSRPPlateSize", VehiclePlateSizeListUrl, _TOKEN);
    GetSizeList("ddlSize", SizeListUrl, _TOKEN);
    $("#btnSave").show();
    $("#btnUpdate").hide();
    pLoadingSetup(true);
});
...
function ClearFormFields() {
    ...
    $("#hdnMapInventoryandHSRPColorID").val(0);

    $('.invalid-feedback').remove();
    $('.form-control').removeClass('is-invalid');

    $("#ddlHSRPPlateSize").val("0").change();
...
$("#btnSave,#btnUpdate").on('click', function () {
    ...
    SizeData.MapInventoryandHSRPSizeID = 0;
    if (this.id == "btnUpdate" && $("#hdnMapInventoryandHSRPSizeID").val() > 0) SizeData.MapInventoryandHSRPSizeID = $("#hdnMapInventoryandHSRPSizeID").val();

    SizeData.SizeID = $('#ddlSize').val();
    SizeData.HSRPPlateSizeID = $('#ddlHSRPPlateSize').val();
    SizeData.IsActive = $("#chkActive").is(':checked') ? true : false;


    if (!SizeData.SizeID) return markInvalid("#ddlColor", "Please Select Size Name");
    if (!SizeData.HSRPPlateSizeID) return markInvalid("#ddlPlateColor", "Please Select HSRP Plate Size");

    SaveandUpdateColor(SizeData);

    return false;
});

function SaveandUpdateColor(SizeData) {
            ...
            success: function (response) {
                ...
                if (response != null && response.result != null) {
                    if (response.result.Success && !response.result.Error) {
                        Swal.fire({
                            title: SizeData.MapInventoryandHSRPSizeID == 0 ? "Saved!" : "Updated!",
                            text: SizeData.MapInventoryandHSRPSizeID == 0 ? SaveSuccessMessage : UpdateSuccessMessage,
                            icon: "success"
                        }).then(() => {
...
function getRecordList() {
    ...
    $('#tblrecordlist').DataTable({
        ...
        "columns": [
            ...
            { "data": "SizeName", "orderable": true, "width": "10%" },
            ...
            },
            {
                ...
                bSortable: false,
                render: function (data, type, row) {
                    return SetActionButtons(row.MapInventoryandHSRPSizeID, _CMPermissions);
                },
                "width": "5%",
                "orderable": false
            },
...
function EditData(id, ViewFlag) {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            $('#divAddEditModal').modal('show');
            var MapInventoryandHSRPSizeIDdata = response.Value;

            $("#hdnMapInventoryandHSRPSizeID").val(MapInventoryandHSRPSizeIDdata.MapInventoryandHSRPSizeID);
            $("#ddlSize").val(MapInventoryandHSRPSizeIDdata.SizeID).change();
            $("#ddlHSRPPlateSize").val(MapInventoryandHSRPSizeIDdata.HSRPPlateSizeID).change();
            $("#chkActive").prop('checked', MapInventoryandHSRPSizeIDdata.IsActive);

            $("#divRecordLog").show();
            $("#spnLastUpdatedBy").html("Last Updated By: " + MapInventoryandHSRPSizeIDdata.LastUpdatedByName);
            $("#spnLastUpdatedDate").html("Date: " + ISTtoLocalTime(MapInventoryandHSRPSizeIDdata.LastUpdatedDate));
...
$('#divAddEditModal').on('shown.bs.modal', function () {
    $('#ddlHSRPPlateSize').select2({ dropdownParent: $('#divAddEditModal'), width: '100%' });
