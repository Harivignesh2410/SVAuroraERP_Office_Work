...
$("#btnAddNew").on('click', function () {
    if (!_CMActionAdd) {
        ...
        $("#divAddEditModal .modal-title").html("<i class='bx bxs-plus-square font-size-20 align-middle me-1'></i>&nbsp;Add New Size");
        ClearFormFields();
...
$("#btnSave,#btnUpdate").on('click', function () {
    ...
    SizeData.SizeID = 0;
    if (this.id == "btnUpdate" && $("#hdnSizeID").val() > 0) SizeData.SizeID = $("#hdnSizeID").val();

    SizeData.SizeName = $('#txtSizeName ').val();
    SizeData.SizeCode = $('#txtSizeCode').val();
    SizeData.IsActive = $("#chkActive").is(':checked') ? true : false;

    //Size Name
    if (!SizeData.SizeName) return markInvalid("#txtSizeName", "Please enter Size Name");
    if (!SizeData.SizeCode) return markInvalid("#txtSizeCode", "Please enter Size Code");
    // If validation fails, keep focus on the first invalid input
    if (!isValid) return;

    SaveandUpdateSize(SizeData);

    return false;
});

function SaveandUpdateSize(SizeData) {
    if (ENABLE_VERBOSE_Logging) //console.log(SizeData);
...
function getRecordList() {
    ...
    $('#tblrecordlist').DataTable({
        ...
        "columns": [
            ...
            { "data": "SizeCode", "orderable": true, "width": "10%" },
            ...
            },
            {
                ...
                bSortable: false,
                render: function (data, type, row) {
                    return SetActionButtons(data.SizeID, _CMPermissions);
                },
                "width": "5%",
                "orderable": false
            },
    ...
    $.ajax({
        ...
        success: function (response) {
            if (ViewFlag) {
                ...
                $("#divAddEditModal .modal-title").html("<i class='fas fa-eye'></i>&nbsp;&nbsp;View Size");
            }
            else {
                $("#divAddEditModal .modal-title").html("<i class='bx bxs-edit font-size-20 align-middle me-1'></i>&nbsp;Edit Size");
                $("#btnSave").hide();
                $("#btnUpdate").show();
            }
            $('#divAddEditModal').modal('show');
            var taxdata = response.Value;
            $("#hdnSizeID").val(taxdata.SizeID);
            $("#txtSizeCode").val(taxdata.SizeCode);
            $("#txtSizeName").val(taxdata.SizeName);
