...
$("#btnSave,#btnUpdate").on('click', function () {
    ...
    var OtherChargesData = new Object();

    OtherChargesData.OtherChargesID = 0;
    if (this.id == "btnUpdate" && $("#hdnOtherChargesID").val() > 0) OtherChargesData.OtherChargesID = $("#hdnOtherChargesID").val();

    OtherChargesData.OtherChargesDescription = $('#txtOtherCharge ').val();
    OtherChargesData.Type = $('#ddltype').val();
    OtherChargesData.IsActive = $("#chkActive").is(':checked') ? true : false;
   
    if (!OtherChargesData.OtherChargesDescription) return markInvalid("#txtOtherCharge", "Please enter Other Charges Description");
    if (OtherChargesData.Type == 0) return markInvalid("#ddltype", "Please Select Type");
    // If validation fails, keep focus on the first invalid input
    if (!isValid) return;

    SaveandUpdateOtherCharges(OtherChargesData);

    return false;
});
function SaveandUpdateOtherCharges(OtherChargesData) {
    if (ENABLE_VERBOSE_Logging) //console.log(OtherChargesData);
    ...
    $.ajax({
        ...
        data: JSON.stringify(OtherChargesData),
        success: function (response) {
            if (ENABLE_VERBOSE_Logging) console.log(response);
            if (response != null && response != null) {
                if (response.Success && !response.Error) {
                    Swal.fire({
                        title: OtherChargesData.OtherChargesID == 0 ? "Saved!" : "Updated!",
                        text: OtherChargesData.OtherChargesID == 0 ? SaveSuccessMessage : UpdateSuccessMessage,
                        icon: "success"
                    }).then(() => {
...
function getRecordList() {
    ...
    $('#tblrecordlist').DataTable({
        ...
        "columns": [
            ...
            { "data": "OtherChargesDescription", "orderable": true },
            { "data": "TypeName", "orderable": true, "width": "10%" },
            {
                "data": "IsActive",
            ...
            },
            {
                ...
                bSortable: false,
                render: function (data, type, row) {
                    return SetActionButtons(data.OtherChargesID, _CMPermissions);
                },
                "width": "5%",
                "orderable": false
            },
...
function EditData(ID, ViewFlag) {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            $('#divAddEditModal').modal('show');
            var OtherChargesData = response.Value;
            $("#hdnOtherChargesID").val(OtherChargesData.OtherChargesID);
            $("#txtOtherCharge").val(OtherChargesData.OtherChargesDescription);
            $("#ddltype").val(OtherChargesData.Type);
            $("#chkActive").prop('checked', OtherChargesData.IsActive);
            $("#divRecordLog").show();
            $("#spnLastUpdatedBy").html("Last Updated By: " + OtherChargesData.LastUpdatedByName);
            $("#spnLastUpdatedDate").html("Date: " + ISTtoLocalTime(OtherChargesData.LastUpdatedDateIST));
