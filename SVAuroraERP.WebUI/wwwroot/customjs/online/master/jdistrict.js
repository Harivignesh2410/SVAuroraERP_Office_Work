...
$("#btnAddNew").on('click', function () {
    ...
    $("#divAddEditModal .modal-title").html("<i class='bx bxs-plus-square font-size-20 align-middle me-1'></i>&nbsp;Add New District");
    ClearFormFields();
...
$("#btnSave,#btnUpdate").on('click', function () {
    ...
    DistrictData.DistrictID = 0;
    if (this.id == "btnUpdate" && $("#hdnDistrictID").val() > 0) DistrictData.DistrictID = $("#hdnDistrictID").val();
    DistrictData.StateID = $('#ddlStateList').val();
    DistrictData.DistrictCode = $('#txtDistrictCode').val();
    DistrictData.DistrictName = $('#txtDistrictName').val();
    DistrictData.IsActive = $("#chkActive").is(':checked') ? true : false;
    if (!DistrictData.StateID || DistrictData.StateID === "0") return markInvalid("#ddlStateList", " Please Select State");
    if (!DistrictData.DistrictCode) return markInvalid("#txtDistrictCode", "Please enter District Code");
    if (!DistrictData.DistrictName) return markInvalid("#txtDistrictName", "Please enter District Name ");
    if (!isValid) return;

    SaveandUpdate(DistrictData);

    return false;
});
function SaveandUpdate(DistrictData) {
...
function getRecordList() {
    ...
    $('#tblrecordlist').DataTable({
        ...
        "columns": [
            ...
            { "data": "DistrictCode", "orderable": true },  
            {
                "data": "IsActive",
            ...
            },
            {
                ...
                bSortable: false,
                render: function (data, type, row) {
                    return SetActionButtons(data.DistrictID, _CMPermissions);
                },
                "orderable": false,
                "width": "5%"
            },
    ...
    $.ajax({
        ...
        success: function (response) {
            if (ViewFlag) {
                ...
                $("#divAddEditModal .modal-title").html("<i class='fas fa-eye'></i>&nbsp;&nbsp;View District");
            }
            else {
                $("#divAddEditModal .modal-title").html("<i class='bx bxs-edit font-size-20 align-middle me-1'></i>&nbsp;Edit District");
                $("#btnSave").hide();
                $("#btnUpdate").show();
            }
            if (response != null && response.Value != null) {
                ...
                if (!result.Error && result.Success && result.ID > 0) {
                    ...
                    var DistrictData = response.Value;
                    $("#hdnDistrictID").val(DistrictData.DistrictID);
                    $("#ddlStateList").val(DistrictData.StateID).change();
                    $("#txtDistrictCode").val(DistrictData.DistrictCode);
                    $("#txtDistrictName").val(DistrictData.DistrictName);
                    $("#chkActive").prop('checked', DistrictData.IsActive);
                    $("#divRecordLog").show();
                    $("#spnLastUpdatedBy").html("Last Updated By: " + DistrictData.LastUpdatedByName);
                    $("#spnLastUpdatedDate").html("Date: " + ISTtoLocalTime(DistrictData.LastUpdatedDate));
                }
                else
