...
$("#btnAddNew").on('click', function () {
    ...
    $("#divAddEditModal .modal-title").html("<i class='bx bxs-plus-square font-size-20 align-middle me-1'></i>&nbsp;Add New Home Fitment Pincode");
    ClearFormFields();
    ...
    if (StateID > 0) {
        $.ajax({
            ...
            success: function (data) {
                $('#ddlDistrictList').empty();
                $("#ddlDistrictList").append("<option value='0'>--Select District--</option>");
                $.each(data.result.Value, function (i, result) {
                    $('#ddlDistrictList').append('<option value="' + result.DistrictID + '">' + result.DistrictName + '</option>');
                });

                $("#ddlDistrictList").val(0).change();
            }
        });
    } else {
        $('#ddlDistrictList').empty();
...
$("#btnSave,#btnUpdate").on('click', function () {
    ...
    var HomeFitmentPincodeData = new Object();

    HomeFitmentPincodeData.HomeFitmentPincodeID = 0;
    if (this.id == "btnUpdate" && $("#hdnHomeFitmentPincodeID").val() > 0) HomeFitmentPincodeData.HomeFitmentPincodeID = $("#hdnHomeFitmentPincodeID").val();
    HomeFitmentPincodeData.StateID = $('#ddlStateList').val();
    HomeFitmentPincodeData.DistrictID = $('#ddlDistrictList').val();
    HomeFitmentPincodeData.Location = $('#txtLocation').val();
    HomeFitmentPincodeData.Pincode = $('#txtPincode').val();
    HomeFitmentPincodeData.IsActive = $("#chkActive").is(':checked') ? true : false;
    if (!HomeFitmentPincodeData.StateID || HomeFitmentPincodeData.StateID === "0") return markInvalid("#ddlStateList", " Please Select State");
    if (!HomeFitmentPincodeData.DistrictID || HomeFitmentPincodeData.DistrictID === "0") return markInvalid("#ddlDistrictList", " Please Select District");
    if (!HomeFitmentPincodeData.Location) return markInvalid("#txtLocation", "Please enter Location");
    if (!HomeFitmentPincodeData.Pincode) return markInvalid("#txtPincode", "Please enter Pincode ");
    if (!isValid) return;

    SaveandUpdate(HomeFitmentPincodeData);

    return false;
});
function SaveandUpdate(HomeFitmentPincodeData) {
    if (ENABLE_VERBOSE_Logging)
        ...
        $.ajax({
            ...
            data: JSON.stringify(HomeFitmentPincodeData),
            success: function (response) {
                if (ENABLE_VERBOSE_Logging) //console.log(response);

                    if (response.Success && !response.Error) {
                        if (HomeFitmentPincodeData.HomeFitmentPincodeID == 0)
                            Swal.fire({ title: "Saved!", text: SaveSuccessMessage, icon: "success", confirmButtonColor: "#556ee6" });
                        else if (HomeFitmentPincodeData.HomeFitmentPincodeID > 0)
    ...
    $('#tblrecordlist').DataTable({
        ...
        "columns": [
            ...
            },
            {
                ...
                bSortable: false,
                render: function (data, type, row) {
                    return SetActionButtons(data.HomeFitmentPincodeID, _CMPermissions);
                },
                "orderable": false,
                "width": "5%"
            },
    ...
    $.ajax({
        ...
        success: function (response) {
            if (ViewFlag) {
                ...
                $("#divAddEditModal .modal-title").html("<i class='fas fa-eye'></i>&nbsp;&nbsp;View Home Fitment Pincode");
            }
            else {
                $("#divAddEditModal .modal-title").html("<i class='bx bxs-edit font-size-20 align-middle me-1'></i>&nbsp;Edit Home Fitment Pincode");
                $("#btnSave").hide();
                $("#btnUpdate").show();
            }
            if (response != null && response.Value != null) {
                ...
                if (!result.Error && result.Success && result.ID > 0) {
                    ...
                    var HomeFitmentPincodeData = response.Value;
                    $("#hdnHomeFitmentPincodeID").val(HomeFitmentPincodeData.HomeFitmentPincodeID);
                    $("#ddlStateList").val(HomeFitmentPincodeData.StateID).change();
                    $("#ddlDistrictList").val(HomeFitmentPincodeData.DistrictID).change();
                    $("#txtLocation").val(HomeFitmentPincodeData.Location);
                    $("#txtPincode").val(HomeFitmentPincodeData.Pincode);
                    $("#chkActive").prop('checked', HomeFitmentPincodeData.IsActive);


                    $("#divRecordLog").show();
                    $("#spnLastUpdatedBy").html("Last Updated By: " + HomeFitmentPincodeData.LastUpdatedByName);
                    $("#spnLastUpdatedDate").html("Date: " + ISTtoLocalTime(HomeFitmentPincodeData.LastUpdatedDate));
                }
                else
