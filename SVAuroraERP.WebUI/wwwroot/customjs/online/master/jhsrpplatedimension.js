...
$("#btnAddNew").on('click', function () {
    ...
    $("#divAddEditModal .modal-title").html("<i class='bx bxs-plus-square font-size-20 align-middle me-1'></i>&nbsp;Add New HSRP Plate Dimension");
    ClearFormFields();
...
function ClearFormFields() {
    ...
    $("#hdnHSRPPlateDimensionID").val(0);

    // Remove previous invalid class
    $('.invalid-feedback').remove();
    $('.form-control').removeClass('is-invalid');

    $("#ddlVehiclePlateColor").val(0);
    $("#ddlVehiclePlateSize").val(0);
...
$("#btnSave,#btnUpdate").on('click', function () {
    ...
    var HSRPPlateDimensionData = new Object();

    HSRPPlateDimensionData.HSRPPlateDimensionID = 0;
    if (this.id == "btnUpdate" && $("#hdnHSRPPlateDimensionID").val() > 0) HSRPPlateDimensionData.HSRPPlateDimensionID = $("#hdnHSRPPlateDimensionID").val();
    HSRPPlateDimensionData.VehiclePlateColorID = $('#ddlVehiclePlateColor').val();
    HSRPPlateDimensionData.VehiclePlateSizeID = $('#ddlVehiclePlateSize').val();
    if (!HSRPPlateDimensionData.VehiclePlateColorID || HSRPPlateDimensionData.VehiclePlateColorID === "0") return markInvalid("#ddlVehiclePlateColor", " Please Select Vehicle Plate Color");
    if (!HSRPPlateDimensionData.VehiclePlateSizeID || HSRPPlateDimensionData.VehiclePlateSizeID === "0") return markInvalid("#ddlVehiclePlateSize", " Please Select Vehicle Plate Size");
    if (!isValid) return;

    SaveandUpdate(HSRPPlateDimensionData);

    return false;
});
function SaveandUpdate(HSRPPlateDimensionData) {
    if (ENABLE_VERBOSE_Logging)
        ...
        $.ajax({
            ...
            data: JSON.stringify(HSRPPlateDimensionData),
            success: function (response) {
                if (ENABLE_VERBOSE_Logging) //console.log(response);

                    if (response.Success && !response.Error) {
                        if (HSRPPlateDimensionData.HSRPPlateDimensionID == 0)
                            Swal.fire({ title: "Saved!", text: SaveSuccessMessage, icon: "success", confirmButtonColor: "#556ee6" });
                        else if (HSRPPlateDimensionData.HSRPPlateDimensionID > 0)
...
function getRecordList() {
    ...
    $('#tblrecordlist').DataTable({
        ...
        "ajax": {
            url: HSRPPlateDimensionListDataURL,
            headers: { "RequestVerificationToken": _TOKEN },
            "type": "POST",
            data: function (d) {
                return {
        ...
        "columns": [
            ...
            { "data": "VehiclePlateColorName", "orderable": true, "width": "10%" },
            { "data": "VehiclePlateSizeName", "orderable": true },
            { "data": "Dimension", "orderable": true },
            {
                data: null,
                bSortable: false,
                render: function (data, type, row) {
                    return SetActionButtons(row.HSRPPlateDimensionID, _CMPermissions);
                },
                "width": "5%",
                "orderable": false
            },
    ...
    $.ajax({
        ...
        success: function (response) {
            if (ViewFlag) {
                ...
                $("#divAddEditModal .modal-title").html("<i class='fas fa-eye'></i>&nbsp;&nbsp;View HSRP Plate Dimension");
            }
            else {
                $("#divAddEditModal .modal-title").html("<i class='bx bxs-edit font-size-20 align-middle me-1'></i>&nbsp;Edit HSRP Plate Dimension");
                $("#btnSave").hide();
                $("#btnUpdate").show();
            }
            if (response != null && response.Value != null) {
                ...
                if (!result.Error && result.Success && result.ID > 0) {
                    ...
                    var HSRPPlateDimensionData = response.Value;
                    $("#hdnHSRPPlateDimensionID").val(HSRPPlateDimensionData.HSRPPlateDimensionID);
                    $("#ddlVehiclePlateColor").val(HSRPPlateDimensionData.VehiclePlateColorID).change();
                    $("#ddlVehiclePlateSize").val(HSRPPlateDimensionData.VehiclePlateSizeID).change();


                    $("#divRecordLog").show();
                    $("#spnLastUpdatedBy").html("Last Updated By: " + HSRPPlateDimensionData.LastUpdatedByName);
                    $("#spnLastUpdatedDate").html("Date: " + ISTtoLocalTime(HSRPPlateDimensionData.LastUpdatedDate));
                }
                else
