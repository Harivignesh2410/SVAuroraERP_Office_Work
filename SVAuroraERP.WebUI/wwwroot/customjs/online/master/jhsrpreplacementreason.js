_PageTitle = "HSRP Replacement Reason";
$(function () {
    pLoadingSetup(false);
    getRecordList();
    $("#btnSave").show();
...
function ClearFormFields() {
    ...
    $("#hdnHSRPReplacementReasonID").val(0);

    // Remove previous invalid class
    $('.invalid-feedback').remove();
    $('.form-control').removeClass('is-invalid');

    $("#txtHSRPReplacementReasonName").val("");
    $("#txtHSRPReplacementReasonCode").val("");
...
$("#btnSave,#btnUpdate").on('click', function () {
    ...
    var HSRPReplacementReasonData = new Object();

    HSRPReplacementReasonData.HSRPReplacementReasonID = 0;
    if (this.id == "btnUpdate" && $("#hdnHSRPReplacementReasonID").val() > 0) HSRPReplacementReasonData.HSRPReplacementReasonID = $("#hdnHSRPReplacementReasonID").val();

    HSRPReplacementReasonData.Code = $('#txtHSRPReplacementReasonCode').val();
    HSRPReplacementReasonData.ReplacementReasonName = $('#txtHSRPReplacementReasonName').val();
    HSRPReplacementReasonData.IsActive = $("#chkActive").is(':checked') ? true : false;
    if (!HSRPReplacementReasonData.Code) return markInvalid("#txtHSRPReplacementReasonCode", "Please enter HSRPReplacementReason Code");
    if (!HSRPReplacementReasonData.ReplacementReasonName) return markInvalid("#txtHSRPReplacementReasonName", "Please enter HSRPReplacementReason Name");
    if (!isValid) return;

    SaveandUpdate(HSRPReplacementReasonData);

    return false;
});
function SaveandUpdate(HSRPReplacementReasonData) {
    if (ENABLE_VERBOSE_Logging) console.log(HSRPReplacementReasonData);
    ...
    $.ajax({
        ...
        data: JSON.stringify(HSRPReplacementReasonData),
        success: function (response) {
            if (ENABLE_VERBOSE_Logging) console.log(response);

            if (response.Success && !response.Error) {
                if (HSRPReplacementReasonData.HSRPReplacementReasonID == 0)
                    Swal.fire({ title: "Saved!", text: SaveSuccessMessage, icon: "success", confirmButtonColor: "#556ee6" });
                else if (HSRPReplacementReasonData.HSRPReplacementReasonID > 0)
    ...
    $('#tblrecordlist').DataTable({
        ...
        "columns": [
            ...
            },
            {
                ...
                bSortable: false,
                render: function (data, type, row) {
                    return SetActionButtons(row.HSRPReplacementReasonID, _CMPermissions);
                },
                "width": "5%",
                "orderable": false
            },
        ...
        success: function (response) {
            ...
            if (response != null && response.Value != null) {
                ...
                if (!result.Error && result.Success && result.ID > 0) {
                    ...
                    var HSRPReplacementReasonData = response.Value;
                    $("#hdnHSRPReplacementReasonID").val(HSRPReplacementReasonData.HSRPReplacementReasonID);
                    $("#txtHSRPReplacementReasonCode").val(HSRPReplacementReasonData.Code);
                    $("#txtHSRPReplacementReasonName").val(HSRPReplacementReasonData.ReplacementReasonName);
                    $("#chkActive").prop('checked', HSRPReplacementReasonData.IsActive);


                    $("#divRecordLog").show();
                    $("#spnLastUpdatedBy").html("Last Updated By: " + HSRPReplacementReasonData.LastUpdatedByName);
                    $("#spnLastUpdatedDate").html("Date: " + ISTtoLocalTime(HSRPReplacementReasonData.LastUpdatedDate));
                }
                else
