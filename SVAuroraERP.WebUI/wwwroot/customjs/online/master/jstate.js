...
$("#btnAddNew").on('click', function () {
    ...
    $("#divAddEditModal .modal-title").html("<i class='bx bxs-plus-square font-size-20 align-middle me-1'></i>&nbsp;Add New State");
    ClearFormFields();
...
$("#btnSave,#btnUpdate").on('click', function () {
    ...
    StateData.StateID = 0;
    if (this.id == "btnUpdate" && $("#hdnStateID").val() > 0) StateData.StateID = $("#hdnStateID").val();

    StateData.StateCode = $('#txtStateCode').val();
    StateData.StateName = $('#txtStateName').val();
    StateData.IsActive = $("#chkActive").is(':checked') ? true : false;
    if (!StateData.StateCode) return markInvalid("#txtStateCode", "Please enter State Code");
    if (!StateData.StateName) return markInvalid("#txtStateName", "Please enter State Name");
    if (!isValid) return;

    SaveandUpdate(StateData);

    return false;
});
function SaveandUpdate(StateData) {
    if (ENABLE_VERBOSE_Logging) console.log(StateData);
...
function getRecordList() {
    ...
    $('#tblrecordlist').DataTable({
        ...
        "columns": [
            ...
            { "data": "StateCode", "orderable": true, "width": "10%" },
            ...
            },
            {
                ...
                bSortable: false,
                render: function (data, type, row) {
                    return SetActionButtons(data.StateID, _CMPermissions);
                },
                "orderable": false,
                "width": "5%"
            },
    ...
    $.ajax({
        ...
        success: function (response) {
            if (ViewFlag) {
                ...
                $("#divAddEditModal .modal-title").html("<i class='fas fa-eye'></i>&nbsp;&nbsp;View State");
            }
            else {
                $("#divAddEditModal .modal-title").html("<i class='bx bxs-edit font-size-20 align-middle me-1'></i>&nbsp;Edit State");
                $("#btnSave").hide();
                $("#btnUpdate").show();
            }
            if (response != null && response.Value != null) {
                ...
                if (!result.Error && result.Success && result.ID > 0) {
                    ...
                    var StateData = response.Value;
                    $("#hdnStateID").val(StateData.StateID);
                    $("#txtStateCode").val(StateData.StateCode);
                    $("#txtStateName").val(StateData.StateName);
                    $("#chkActive").prop('checked', StateData.IsActive);


                    $("#divRecordLog").show();
                    $("#spnLastUpdatedBy").html("Last Updated By: " + StateData.LastUpdatedByName);
                    $("#spnLastUpdatedDate").html("Date: " + ISTtoLocalTime(StateData.LastUpdatedDate));
                }
                else
