...
$("#btnAddNew").on('click', function () {
    ...
    $("#divAddEditModal .modal-title").html("<i class='bx bxs-plus-square font-size-20 align-middle me-1'></i>&nbsp;Add New Vehicle Class");
    ClearFormFields();
...
function ClearFormFields() {
    ...
    $("#txtVehicleClassCode").val("");
    $("#txtVehicleClassName").val("");
...
$("#btnSave,#btnUpdate").on('click', function () {
    ...
    var VehicleClassData = new Object();

    VehicleClassData.VehicleClassID = 0;
    if (this.id == "btnUpdate" && $("#hdnVehicleClassID").val() > 0) VehicleClassData.VehicleClassID = $("#hdnVehicleClassID").val();

    VehicleClassData.VehicleClassCode = $('#txtVehicleClassCode ').val();
    VehicleClassData.VehicleClassName = $('#txtVehicleClassName').val();
    VehicleClassData.IsActive = $("#chkActive").is(':checked') ? true : false;
    if (!VehicleClassData.VehicleClassCode) return markInvalid("#txtVehicleClassCode", "Please enter Vehicle Class Code");
    if (!VehicleClassData.VehicleClassName) return markInvalid("#txtVehicleClassName", "Please enter Vehicle Class Name");
    if (!isValid) return;

    SaveandUpdate(VehicleClassData);

    return false;
});
function SaveandUpdate(VehicleClassData) {
    if (ENABLE_VERBOSE_Logging) console.log(VehicleClassData);
...
function getRecordList() {
    ...
    $('#tblrecordlist').DataTable({
        ...
        "columns": [
            ...
            { "data": "VehicleClassCode", "orderable": true, "width": "10%" },
            ...
            },
            {
                ...
                bSortable: false,
                render: function (data, type, row) {
                    return SetActionButtons(row.VehicleClassID, _CMPermissions);
                },
                "width": "5%",
                "orderable": false
            },
    ...
    $.ajax({
        ...
        success: function (response) {
            if (ViewFlag) {
                ...
                $("#divAddEditModal .modal-title").html("<i class='bx bxs-edit font-size-20 align-middle me-1'></i>&nbsp;Edit Vehicle Class");
                $("#btnSave").hide();
                $("#btnUpdate").show();
            }
            if (response != null && response.Value != null) {
                ...
                if (!result.Error && result.Success && result.ID > 0) {
                    ...
                    var VehicleClassData = response.Value;
                    $("#hdnVehicleClassID").val(VehicleClassData.VehicleClassID);
                    $("#txtVehicleClassCode").val(VehicleClassData.VehicleClassCode);
                    $("#txtVehicleClassName").val(VehicleClassData.VehicleClassName);
                    $("#chkActive").prop('checked', VehicleClassData.IsActive);


                    $("#divRecordLog").show();
                    $("#spnLastUpdatedBy").html("Last Updated By: " + VehicleClassData.LastUpdatedByName);
                    $("#spnLastUpdatedDate").html("Date: " + ISTtoLocalTime(VehicleClassData.LastUpdatedDate));
                }
                else
