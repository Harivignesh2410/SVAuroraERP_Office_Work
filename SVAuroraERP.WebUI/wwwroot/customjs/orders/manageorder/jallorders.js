    ...
    let table = $('#tblrecordlist').DataTable({
        ...
        columns: [
            ...
            },
            {
                data: null,
                render: function (data, type, row) {
                    return HsrpGroupActionForAllOrder(row.HSRPOrderID, row.OrderStatusID);
                },
                orderable: false,
            ...
            {
                "data": "Description", "orderable": true, "width": "10%", "className": "text-center text-light",
                "render": function (data, type, row) {
            ...
            },
            {
                ...
                render: function (data, type, row) {
                    return `
                          <span >${row.OEM}</span><br>
                          <span >${row.OEMCode}</span>
                           <span > - ${row.OEMCity}</span>
                        `;
                }
            },
            {
                ...
                render: function (data, type, row) {
                    return `
                          <span >${row.EmbossingStation}</span><br>
                          <span >${row.EmbossingStationCode}</span>
                           <span> - ${row.EmbossingStationCity}</span>
                        `;
                }
            },
            {
...
$("#btnExport").on('click', function () {
    ...
    $.ajax({
        ...
        success: function (data, status, xhr) {
            var filename = "HSRPOrderData-" + new Date().toISOString().replace(/T/, '_').replace(/\..+/, '') + ".xlsx";
        ...
        error: function (xhr, status, error) {
            //console.error("Export failed:", xhr.responseText || error);
