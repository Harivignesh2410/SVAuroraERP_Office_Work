    ...
    let table = $('#tblrecordlist').DataTable({
        ...
        `;
            ...
            },
            {
                ...
                render: function (data, type, row) {
                    return `
                <span>${row.DocketNo}</span><br>
                <span>${row.ModeOfTransport}</span><br>
...
function GetHSRPDataByID(HSRPOrderID) {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            $("#divDeliveryAcknowledgementModal .modal-title").html("<i class='bx bxs-plus-square font-size-20 align-middle me-1'></i>&nbsp;Delivery Acknowledgement");
...
$('#btnDeliveryAcknowledgementCheck').on('click', async function () {
    ...
    let frontUpload = await UploadFrontImage(OrderYear,OrderMonth);
    let rearUpload = await UploadRearImage(OrderYear, OrderMonth);

    var DeliveryAcknowledgementData = {};

    DeliveryAcknowledgementData.OrderID = HSRPOrderArray.HSRPOrderID;
    // DeliveryAcknowledgementData.OrderDate = HSRPOrderArray.OrderDate;


    DeliveryAcknowledgementData.FrontLaserNoURL = frontUpload.success ? frontUpload.filePath : null;
    DeliveryAcknowledgementData.RearLaserNoURL = rearUpload.success ? rearUpload.filePath : null;


    if (!DeliveryAcknowledgementData.FrontLaserNoURL) return markInvalid("#FrontImg", "Please Upload the Front Image");
    if (!DeliveryAcknowledgementData.RearLaserNoURL) return markInvalid("#RearImg", "Please Upload the Rear Image");
