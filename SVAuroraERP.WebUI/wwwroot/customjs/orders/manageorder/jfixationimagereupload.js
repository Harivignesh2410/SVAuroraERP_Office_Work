        ...
        columns: [
...

            {
                ...
                render: function (data, type, row) {
                    return HsrpGroupActionForDeliveryAcknowledgementOrders(row.HSRPOrderID);
                },
                orderable: false,
...
function GetHSRPDataByID(HSRPOrderID) {
    ...
    $.ajax({
        ...
        success: function (response) {
            $("#divDeliveryAcknowledgementModal .modal-body :input").attr("disabled", false);
            $("#divDeliveryAcknowledgementModal .modal-title").html("<i class='bx bxs-plus-square font-size-20 align-middle me-1'></i>&nbsp; Fitted Image Re-Upload");
            $("#btnDeliveryAcknowledgementCheck").show();
            $('#divDeliveryAcknowledgementModal').modal('show');
            if (response) {
                HSRPOrderArray = response;
                // -------- FRONT IMAGE --------
                if (response.FrontLaserNoURL) {
                                    $("#divfrontImage").html(`
                        <h6 class="text-primary">Front Image</h6>
                        <img id="frontImagePreview"
                             src="${response.FrontLaserNoURL}"
                             class="img-fluid border rounded mb-3"
                             style="max-width:550px; cursor:zoom-in;"
                    ...
                    `);
                                    ...
                                    $("#frontImagePreview")
                                        ...
                                        .on("click", function () {
                                            openImageZoom(
                                                response.FrontLaserNoURL,
                                                "Front Plate Image"
                                            );
                                        });
                                } else {
                                    $("#divfrontImage").html(`<p class="text-muted">No front image available</p>`);
                                }

                                // -------- REAR IMAGE --------
                                if (response.RearLaserNoURL) {
                                    $("#divRearImage").html(`
                        <h6 class="text-primary">Rear Image</h6>
                        <img id="rearImagePreview"
                             src="${response.RearLaserNoURL}"
                             class="img-fluid border rounded"
                             style="max-width:550px; cursor:zoom-in;"
                             alt="Rear Plate Image">
                    `);
                    ...
                    $("#rearImagePreview")
                        ...
                        .on("click", function () {
                            openImageZoom(
                                response.RearLaserNoURL,
                                "Rear Plate Image"
                            );
                        });
                } else {
                    $("#divRearImage").html(`<p class="text-muted">No rear image available</p>`);
                }
                ...
                OrderdataContent += `
                            <tr>
                                <td>${response.Dealer},<br>${response.DealerCode},<br>${response.DealerCity}</td>
                               <td>${response.OEM},<br>${response.OEMCode},<br>${response.OEMCity}</td>
                                 <td>${response.EmbossingStation},<br>${response.EmbossingStationCode},<br>${response.EmbossingStationCity}</td>
                                <td>${response.OrderNo}</td>
                                <td>${response.sOrderDate}<br>${response.sRegDate}</td>                          
                            </tr>`;
...
$('#btnDeliveryAcknowledgementCheck').on('click', async function () {

    $('.invalid-feedback').remove();
    $('.form-control').removeClass('is-invalid');

    let uploadType = $('input[name="imgUploadType"]:checked').val();

    var orderDate = new Date(HSRPOrderArray.OrderDate);
    var OrderYear = orderDate.getFullYear();
    var OrderMonth = String(orderDate.getMonth() + 1).padStart(2, '0');

    let frontUpload = { success: false };
    let rearUpload = { success: false };

    if (uploadType === "Front" || uploadType === "Both") {
        frontUpload = await UploadFrontImage(OrderYear, OrderMonth);
        if (!frontUpload.success)
            return markInvalid("#FrontImg", "Please upload Front Image");
    }

    if (uploadType === "Rear" || uploadType === "Both") {
        rearUpload = await UploadRearImage(OrderYear, OrderMonth);
        if (!rearUpload.success)
            return markInvalid("#RearImg", "Please upload Rear Image");
    }

    var DeliveryAcknowledgementData = {
        OrderID: HSRPOrderArray.HSRPOrderID,
        FrontLaserNoURL: frontUpload.success ? frontUpload.filePath : null,
        RearLaserNoURL: rearUpload.success ? rearUpload.filePath : null
    };

   SaveandUpdate(DeliveryAcknowledgementData);
});

function UploadFrontImage(OrderYear, OrderMonth) {
...
function UploadRearImage(OrderYear, OrderMonth) {
...
function SaveandUpdate(DeliveryAcknowledgementData) {
    $.ajax({
        url: SaveUpdateDataUrl,
        type: 'POST',
        headers: { "RequestVerificationToken": _TOKEN },
        contentType: 'application/json',
        data: JSON.stringify(DeliveryAcknowledgementData),
        success: function (response) {
            if (response && response.dataResponse) {
                ...
                if (!result.Error && result.Success) {
                    Swal.fire({
                        ...
                        $('#divDeliveryAcknowledgementModal').modal('hide');
                        ClearFormFields();
                        getSummaryList();
                        $("#btnFilter").click();
                    });
...
$('input[name="imgUploadType"]').on('change', function () {
    let type = $(this).val();

    $("#FrontImg, #RearImg").prop("disabled", false);

    if (type === "Front") {
        $("#RearImg").val("").prop("disabled", true);
    }
    else if (type === "Rear") {
        $("#FrontImg").val("").prop("disabled", true);
    }
});
