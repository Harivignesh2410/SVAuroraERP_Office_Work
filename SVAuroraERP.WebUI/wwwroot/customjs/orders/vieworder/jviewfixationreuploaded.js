_PageTitle = "Fixation ReUploaded";
let flatData = [];
let selectedIds = [];
var HSRPOrderArray = [];
                        ...
                        <li>
                            <a class="dropdown-item d-flex align-items-center btn-vahan-api"
                               ...
                               data-id="${row.HSRPOrderID}">
                                <i class="bx bx-cloud-upload text-primary me-2"></i> Review
                            </a>
                        </li>
            ...
            },
            {
                ...
                render: function (url) {
                    ...
                    <a href="${url}" class="image-popup">
                        <img src="${url}" class="img-thumbnail" style="max-height:60px;" />
                    </a>`;
                }
            }
            ,
...
//$(document).on("click", ".btn-vahan-submit", function () {
//    let id = $(this).data("id");

//    $("#btnVahanSubmitProceed").data("id", id);
//    $("#btnVahanSubmitProceed").data("isSubmit", true);

//    LoadVehicleImageData(id, "submit");
//});
...
//    $("#btnRejectProceed").data("id", id);
//    $("#btnRejectProceed").data("isSubmit", false);

//    LoadVehicleImageData(id, "reject");
//});


//$(document).on("click", "#btnVahanSubmitProceed", function () {
//    let id = $(this).data("id");
//    let isSubmit = $(this).data("isSubmit");

//    let Data = {
//        HSRPVehiclePlateImageID: id,
//        IsSubmit: isSubmit
//    };

//    VahanSubmitAction(Data);
//});

//$(document).on("click", "#btnRejectProceed", function () {
//    let id = $(this).data("id");
//    let isSubmit = $(this).data("isSubmit");
...
//    $.ajax({
//        url: SaveFixationReUploadedsUrl,
//        type: "POST",
//        headers: { "RequestVerificationToken": _TOKEN },
//        contentType: 'application/json',
...
//        success: function (response) {
//            $("#modalVahanSubmit").modal("hide");
//            $("#modalReject").modal("hide");
//            $("#btnRefresh").click();
//            Swal.fire({
//                title: "Success",
//                text: response.message || (Data.IsSubmit ? "Submitted successfully!" : "Rejected successfully!"),
//                icon: "success",
...
//function LoadVehicleImageData(ID, type) {
//    $.ajax({
...
//                // FRONT IMAGE
//                if (data.FrontLaserNoURL && data.FrontLaserNoURL.trim() !== "") {
//                    $("#frontImagePreview")
//                        .attr("src", data.FrontLaserNoURL)
//                        .show();

//                    $("#frontImageLink")
//                        .attr("href", data.FrontLaserNoURL)
//                        .attr("title", "Front Image");
//                } else {
//                    $("#frontImagePreview").hide();
//                }

//                // REAR IMAGE
//                if (data.RearLaserNoURL && data.RearLaserNoURL.trim() !== "") {
//                    $("#rearImagePreview")
//                        .attr("src", data.RearLaserNoURL)
//                        .show();

//                    $("#rearImageLink")
//                        .attr("href", data.RearLaserNoURL)
//                        .attr("title", "Rear Image");
//                } else {
//                    $("#rearImagePreview").hide();
//                }

//                // Last Updated info
//                $("#spnLastUpdatedBy").html("Last Updated By: " + data.LastUpdatedByName);
//                $("#spnLastUpdatedDate").html("Date: " + ISTtoLocalTime(data.LastUpdatedDate));

//                // OPEN CORRECT MODAL
//                if (type === "submit") {
//                    $("#modalVahanSubmit").modal("show");
//                } else if (type === "reject") {
//                    $("#modalReject").modal("show");
//                }
//            }
//            else {
//                Swal.fire("Error", "Data not found!", "warning");
//            }
//        },
//        error: function (xhr) {
...
function LoadVehicleImageData(ID) {
    $.ajax({
        ...
        success: function (response) {
            if (response && response.Value) {
                let FixationReUploadedData = response.Value;
                if (FixationReUploadedData) {
                    HSRPOrderArray = FixationReUploadedData;
                    $("#divOrderdata").empty();
                    ...
                    OrderdataContent += `
                            <tr>
                                <td>${FixationReUploadedData.Dealer},<br>${FixationReUploadedData.DealerCode},<br>${FixationReUploadedData.DealerCity}</td>
                               <td>${FixationReUploadedData.OEM},<br>${FixationReUploadedData.OEMCode},<br>${FixationReUploadedData.OEMCity}</td>
                                 <td>${FixationReUploadedData.EmbossingStation},<br>${FixationReUploadedData.EmbossingStationCode},<br>${FixationReUploadedData.EmbossingStationCity}</td>
                                <td>${FixationReUploadedData.OrderNo}</td>
                                <td>${FixationReUploadedData.sOrderDate}<br>${FixationReUploadedData.sRegDate}</td>                          
                            </tr>`;
                    ...
                    FtableContent += `
                            <tr>
                                <td>${FixationReUploadedData.RegNo}</td>
                                <td>${FixationReUploadedData.FrontLaserSerialNo}</td>
                                <td>${FixationReUploadedData.FrontPlateDimension}</td>
                            </tr>`;
                    ...
                    RtableContent += `
                            <tr>
                                <td>${FixationReUploadedData.RegNo}</td>
                                <td>${FixationReUploadedData.RearLaserSerialNo}</td>
                                <td>${FixationReUploadedData.RearPlateDimension}</td>
                            </tr>`;
                    ...
                    if (FixationReUploadedData.FrontLaserNoURL) {
                        $("#frontImagePreview").attr("src", FixationReUploadedData.FrontLaserNoURL).show();
                        //$("#frontImageLink").attr("href", FixationReUploadedData.FrontLaserNoURL);
                    } else {
                        $("#frontImagePreview").hide();
                    }

                    // REAR IMAGE
                    if (FixationReUploadedData.RearLaserNoURL) {
                        $("#rearImagePreview").attr("src", FixationReUploadedData.RearLaserNoURL).show();
                        // $("#rearImageLink").attr("href", FixationReUploadedData.RearLaserNoURL);
                    } else {
                        $("#rearImagePreview").hide();
                    }
...
$(document).on("click", "#btnVahanSubmit", function () {
    ...
    let frontNo = $("#txtFrontRegNo").val().trim();
    let rearNo = $("#txtRearRegNo").val().trim();

    let originalNo = HSRPOrderArray.RegNo;

    if (frontNo === "" || rearNo === "") {
        $.jGrowl("Please enter both Front & Rear Reg No!", { theme: 'danger' });
        return;
    }

    if (frontNo !== originalNo || rearNo !== originalNo) {
        $.jGrowl("Front & Rear Reg No must match the original Reg No!", { theme: 'danger' });
        return;
    }
